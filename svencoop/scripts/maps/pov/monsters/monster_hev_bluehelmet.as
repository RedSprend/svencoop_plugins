...
const int HEVBLUE_AE_DRAW		= 2;
const int HEVBLUE_AE_SHOOT		= 3;
const int HEVBLUE_AE_HOLSTER		= 4;
...
class monster_hev_bluehelmet : ScriptBaseMonsterEntity
{
	...
	monster_hev_bluehelmet()
	{
		@this.m_Schedules = @monster_hev_bluehelmet_schedules;
	}
	...
	void HandleAnimEvent( MonsterEvent@ pEvent )
	{
		switch( pEvent.event )
		{
		case HEVBLUE_AE_SHOOT:
			FirePistol();
			break;
		case HEVBLUE_AE_DRAW:
			// barney's bodygroup switches here so he can pull gun from holster
			self.SetBodygroup( BODYGROUP_WEAPONS, GUN_PISTOLDRAWN );
			m_fGunDrawn = true;
			break;
		case HEVBLUE_AE_HOLSTER:
	...
	void Precache()
	{
		...
		{
			g_Game.PrecacheModel( "models/hlclassic/pov/hev_barney.mdl" );
			g_Game.PrecacheGeneric( "models/hlclassic/pov/hev_barney01.mdl" );
			g_Game.PrecacheGeneric( "models/hlclassic/pov/hev_barney02.mdl" );
			g_Game.PrecacheGeneric( "models/hlclassic/pov/hev_barney03.mdl" );
			g_Game.PrecacheGeneric( "models/hlclassic/pov/hev_barneyt.mdl" );
		}
		else
	...
	void Spawn()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
			g_EntityFuncs.SetModel( self, "models/hlclassic/pov/hev_barney.mdl" );
		else
		...
		{
			self.m_FormattedName = "HEV Barney";
		}
		...
		case MONSTERSTATE_COMBAT:
			{
				...
				if( self.HasConditions ( bits_COND_NO_AMMO_LOADED ) )
					return self.GetScheduleOfType ( SCHED_HEVBLUERELOAD );
			}
			break;
...
array<ScriptSchedule@>@ monster_hev_bluehelmet_schedules;
...
void InitSchedules()
{
	...
	@monster_hev_bluehelmet_schedules = @scheds;
}

enum monsterScheds
{
	SCHED_HEVBLUERELOAD = LAST_COMMON_SCHEDULE + 1,
}

void Register()
{
	InitSchedules();
	g_CustomEntityFuncs.RegisterCustomEntity( "HEVBLUE::monster_hev_bluehelmet", "monster_hev_bluehelmet" );
}
