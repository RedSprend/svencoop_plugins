namespace HEVORANGE_XTBU
{
const int ACTIVITY_NOT_AVAILABLE 	= -1;

const int HEVORANGE_XTBU_AE_DRAW		= 2;
const int HEVORANGE_XTBU_AE_SHOOT		= 3;
const int HEVORANGE_XTBU_AE_HOLSTER		= 4;
...
class monster_hev_orangehelmet_xtbu : ScriptBaseMonsterEntity
{
	...
	monster_hev_orangehelmet_xtbu()
	{
		@this.m_Schedules = @monster_hev_orangehelmet_xtbu_schedules;
	}
	...
	void HandleAnimEvent( MonsterEvent@ pEvent )
	{
		switch( pEvent.event )
		{
		case HEVORANGE_XTBU_AE_SHOOT:
			FirePistol();
			break;
		case HEVORANGE_XTBU_AE_DRAW:
			// barney's bodygroup switches here so he can pull gun from holster
			self.SetBodygroup( BODYGROUP_WEAPONS, GUN_PISTOLDRAWN );
			m_fGunDrawn = true;
			break;
		case HEVORANGE_XTBU_AE_HOLSTER:
	...
	{		
		...
		case SCHED_HEVORANGE_XTBURELOAD:
			return slReload;
	...
	{
		g_EngineFuncs.ServerPrint("-- DEBUG: SetActivity\n");
		int iSequence = ACTIVITY_NOT_AVAILABLE;

		switch( NewActivity )
		{
			case ACT_RANGE_ATTACK1:
			g_EngineFuncs.ServerPrint("-- DEBUG: ACT_RANGE_ATTACK1\n");
			...
			/*if( self.GetBodygroup( BODYGROUP_WEAPONS ) == (GUN_PISTOLHOLSTERED|GUN_PISTOLDRAWN) )
			{
				iSequence = self.LookupSequence( "shootgun" );
			}*/
			break;
		...
		case MONSTERSTATE_COMBAT:
			{
				...
				if( self.HasConditions ( bits_COND_NO_AMMO_LOADED ) )
					return self.GetScheduleOfType ( SCHED_HEVORANGE_XTBURELOAD );
			}
			break;
...
array<ScriptSchedule@>@ monster_hev_orangehelmet_xtbu_schedules;
...
void InitSchedules()
{
	...
	@monster_hev_orangehelmet_xtbu_schedules = @scheds;
}

enum monsterScheds
{
	SCHED_HEVORANGE_XTBURELOAD = LAST_COMMON_SCHEDULE + 1,
}

void Register()
{
	InitSchedules();
	g_CustomEntityFuncs.RegisterCustomEntity( "HEVORANGE_XTBU::monster_hev_orangehelmet_xtbu", "monster_hev_orangehelmet_xtbu" );
}
