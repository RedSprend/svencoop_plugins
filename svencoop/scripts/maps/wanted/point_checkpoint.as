...
class point_checkpoint : ScriptBaseAnimating
{
	...
	private float m_flDelayBeforeStart 			= 0;

	// Time between player revive
	private float m_flDelayBetweenRevive 		= 0.8f;
	...
	void Precache()
	{
		...
		g_SoundSystem.PrecacheSound( "../sound/wanted/media/checkpoint.mp3" );
		g_Game.PrecacheGeneric( "sound/wanted/media/checkpoint.mp3" );
	...
	void Spawn()
	{
		...
		self.pev.scale 			= 2;
		self.pev.angles 		= Vector(90,0,0);
		self.pev.renderfx		= kRenderFxGlowShell;
		self.pev.rendermode		= kRenderNormal;
		switch( Math.RandomLong(0,2) )
		{
		case 0: self.pev.rendercolor = Vector(128,0,0); break;
		case 1: self.pev.rendercolor = Vector(0,128,0); break;
		case 2: self.pev.rendercolor = Vector(0,0,128); break;
		}
		self.pev.renderamt 		= 8;
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "../sound/wanted/media/checkpoint.mp3", 1.0f, ATTN_NONE );
	...
	void RespawnThink()
	{
		...
		/*NetworkMessage m( MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY, null );
			...
			m.WriteCoord( Math.RandomFloat(-100,100) );
			m.WriteCoord( Math.RandomFloat(-800,800) );
			m.WriteCoord( Math.RandomFloat(-360,360) );
			m.WriteAngle( Math.RandomFloat(-360,360) );
			m.WriteShort( g_EngineFuncs.ModelIndex("models/wanted/chicken.mdl") );
			m.WriteByte( 2 );
			m.WriteByte( 150 );
		m.End();
		...
		if( m_iNextPlayerToRevive > g_Engine.time )
			return;*/
